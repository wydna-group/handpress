...
(define lower-bill
  (hash #\e 2907 #\t 1606 #\o 1588 #\a 1479 #\r 1230 #\h 1275
        ...
        ;; Smith tabulates no thin for a fount without one, and the rule is that
        ;; the work still has to be done by the bodies that exist. His three fine
        ;; spaces did it between them; Moxon has one, so it does it alone, and
        ;; the total space-metal in the case is unchanged. Roadmap §4.
...
;; of the sizes he has: an em quad, an en, a thick, a thin. Largest first,
;; because that is fewest pieces and fewest chances of one working loose.
;;
...
(module+ test
  ...
  (let ([tc6 (make-type-case #:rng (make-rng 1608))])
    ...
    (check-false short?)
    ;; a thick and a thin -- the two spaces Moxon's fount has, whatever the
    ;; unit happens to be
